...
if ( $osid == 0 ) {
	$sql = "insert into sensor (sensor_id, sensor_name, add_info, node_id, channel, html_show, html_order, fhem_dev, store_days) values(".$sid.",'".$sn."', '".$si."', ".$nid.", ".$ch.", '".$sh."' , ".$so. ", '".$fh."' , ".$sd.")";
	$rf24hub_db->query($sql);
	$sql = "insert into sensor_im (sensor_id) values (".$sid.")";
	$rf24hub_db->query($sql);
	print "Neuen Sensor angelegt: ".$sid." SQL: ".$sql;
} else {
	$sql = "update sensor set sensor_name = '".$sn."', add_info = '".$si."', node_id = ".$nid.", channel = ".$ch.", html_show = '".$sh."', html_order = ".$so.", fhem_dev = '". $fh ."', store_days = ".$sd. " where sensor_id = ".$osid;
    $rf24hub_db->query($sql);
