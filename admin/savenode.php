...
if (isset($_GET["show"])) { $ns=$_GET["show"]; } else { $ns="n"; }
...
$sql ="insert into node (node_id, node_name, add_info, battery_id, mastered, html_show) values('".$nid."', '".$nn."', '".$ni."', ".$bid.", 'y', 'y')";
	...
	$sql = " update node set node_name = '".$nn."', add_info = '".$ni."', battery_id = ".$bid.", mastered = '". $ms."', html_order = '". $sort."', html_show = '".$ns."'  where node_id = '".$onid."' ";
        $rf24hub_db->query($sql);
